...
bool Separation::separate( int N, int M, vector<edge> E, double sol[], double C[MAXN][MAXN], double W[MAXN][MAXN], 
	...
	/* Add the capacities to the extra vertex */
	int root = ( N & 1 )? N:(N+1);
	W[root][root] = 0.0;
	for ( int i=0; i < N; ++i ){
		W[root][i] = 0.0;
		for ( int j=0; j < N; ++j ){
			if ( i != j ) W[root][i] += W[i][j];
		}
		W[root][i] -= 1.0;
		W[i][root] = W[root][i];
	}
	if ( N & 1 ){ // N is odd (just add one vertex)
		return min_odd_cut( N + 1, M, E, sol, W, coef, sz );
	}
	else {
		for ( int i=0; i < N; ++i ){
			W[N][i] = W[i][N] = 0.0;
		}
		W[N][root] = W[root][N] = 2 * N * N; // Big value 
		return min_odd_cut( N + 2, M, E, sol, W, coef, sz );
	}
	return false;
} 
